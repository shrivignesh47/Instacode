  ...
  try {
    ...
    for (const testCase of testCases) {
      try {
        ...
        if (errorOutput) {
          status = result.compile_output ? "compilation_error" : "runtime_error";
        ...
        testResults.push({
          ...
          actual_output: actualOutput,
          passed,
          execution_time_ms: result.time ? result.time * 1000 : 0,
          memory_used_mb: result.memory ? result.memory / 1024 : 0,
          is_sample: testCase.is_sample
        });

        totalExecutionTime += result.time ? result.time * 1000 : 0;
...
function mapLanguageToJudge0Id(language) {
  ...
  return mapping[language] || 71; // fallback to Python
}
