...
const PostGrid = ({
  ...
  return (
    <div className="mb-6 sm:mb-8 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 lg:gap-6 auto-rows-max">
          ...
          return (
            <div 
              ...
              className={`bg-gray-800 rounded-lg sm:rounded-xl border border-gray-700 overflow-hidden hover:border-gray-600 hover:shadow-xl transition-all duration-300 ${
                isWidePost ? 'col-span-1 lg:col-span-2 xl:col-span-3' : 'col-span-1'
              }`}
            >
              {/* Post Header */}
                ...
                {post.media_url && (
                  <img
                    ...
                    className={`w-full object-cover ${
                      isWidePost 
                        ? 'h-32 sm:h-48 lg:h-64 xl:h-80' 
                        : 'h-32 sm:h-40 lg:h-48 xl:h-64'
                    }`}
                  />
                )}
                ...
                {post.type === 'code' && post.code_content && post.code_language && (
                  <div className="bg-gray-900 rounded-lg overflow-hidden mb-3 sm:mb-4 border border-gray-600">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-3 sm:px-4 py-2 sm:py-3 bg-gray-700 gap-2 sm:gap-3">
                      ...
                      <div className="flex items-center space-x-2 w-full sm:w-auto">
                        <button
                          ...
                          className="flex items-center justify-center space-x-1 px-3 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded-lg transition-colors font-medium flex-1 sm:flex-initial"
                        >
                        ...
                        {!useAdvancedEditor[post.id] && (
                          <button
                            ...
                            className="flex items-center justify-center space-x-1 px-3 py-1.5 sm:py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white text-xs rounded-lg transition-colors font-medium flex-1 sm:flex-initial"
                          >
                    ...
                    {useAdvancedEditor[post.id] ? (
                      ...
                      <>
                        <div className="overflow-x-auto text-xs sm:text-sm max-h-48 sm:max-h-64 lg:max-h-96 overflow-y-auto">
                          <SyntaxHighlighter
                            language={post.code_language}
                            style={oneDark}
                            customStyle={{
                  ...
                  <div className="bg-gray-700 rounded-lg overflow-hidden mb-3 sm:mb-4">
                    {post.project_title && (
                      <div className="p-3 sm:p-4 lg:p-6 border-b border-gray-600">
                        <h5 className="text-base sm:text-lg lg:text-2xl font-semibold text-white mb-2 sm:mb-3">{post.project_title}</h5>
                        {post.project_description && (
                          <p className="text-gray-300 text-sm sm:text-base lg:text-lg leading-relaxed">{post.project_description}</p>
                        )}
                      </div>
                    )}
                      ...
                      <div className="px-3 sm:px-4 lg:px-6 py-3 sm:py-4 border-b border-gray-600">
                        <div className="flex flex-wrap gap-1.5 sm:gap-2">
                          {post.project_tech_stack.slice(0, 6).map((tech: string, index: number) => (
                            <span
                              key={index}
                              className="px-2 py-1 sm:px-3 sm:py-1 bg-gray-600 text-gray-200 text-xs sm:text-sm rounded-lg font-medium"
                            >
                              {tech}
                            </span>
                          ))}
