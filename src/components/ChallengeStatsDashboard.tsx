...
interface ChallengeStatsDashboardProps {
  userId?: string; // If not provided, uses current user
  isCompact?: boolean;
}

const ChallengeStatsDashboard: React.FC<ChallengeStatsDashboardProps> = ({ 
  userId,
  isCompact = false
}) => {
  const { user } = useAuth();
  const [stats, setStats] = React.useState<ChallengeStats | null>(null);
  ...
  React.useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Aggregate stats from user_challenge_stats
        ...
        const aggregatedStats = {
          total_solved: data.filter(stat => stat.solved).length,
          easy_solved: data.filter(stat => stat.solved && stat.challenges?.difficulty === 'easy').length,
          medium_solved: data.filter(stat => stat.solved && stat.challenges?.difficulty === 'medium').length,
          hard_solved: data.filter(stat => stat.solved && stat.challenges?.difficulty === 'hard').length,
        ...
        setError(err.message || 'Failed to load challenge statistics');
      } finally {
        setLoading(false);
      }
    };
...
export default ChallengeStatsDashboard;