...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createDefaultUser = (supabaseUser: SupabaseUser): User => {
    ...
    return {
      ...
      avatar: 'https://ia801307.us.archive.org/1/items/instagram-plain-round/instagram%20dip%20in%20hair.jpg',
      bio: '',
      githubUrl: '',
      linkedinUrl: '',
      twitterUrl: '',
      ...
      try {
        ...
        if (profile) {
          ...
          return {
            ...
            email: profile.email,
            avatar: profile.avatar_url || 'https://ia801307.us.archive.org/1/items/instagram-plain-round/instagram%20dip%20in%20hair.jpg',
            bio: profile.bio || '',
  ...
  const createUserProfile = async (supabaseUser: SupabaseUser, username: string): Promise<void> => {
    try {
      const { error } = await supabase.from('profiles').insert({
        ...
        username,
        email: supabaseUser.email || '',
        avatar_url: 'https://ia801307.us.archive.org/1/items/instagram-plain-round/instagram%20dip%20in%20hair.jpg',
